...
export function ConnectCard({ onConnect, error }: { onConnect: (code: string) => void; error: string }) {
  ...
  return (
    <Card
      title={t("공유 시간표 연결")}
      desc={t("센터 투데이에서 쓰는 공유 공간 접속 코드를 넣으면 관리자가 올린 시간표를 모두 같이 봅니다.")}
    >
      <form
        ...
        onSubmit={(e) => {
          ...
          if (!validCode(next)) return setLocal(t("48자리 접속 코드를 확인해 주세요."));
          setLocal("");
          onConnect(next);
        }}
      >
...
export function PublishDialog({ code, draft, shared, onPublished, onLatest, onClose }: PublishProps) {
  ...
  const pick = (file: File) => {
    ...
    file
      ...
      .then((text) => {
        ...
        if (!parsed) throw new Error(t("시간표 설정 파일이 아닙니다."));
        setFileData(parsed);
      ...
      .catch((e: unknown) => {
        setFileData(null);
        setError(t("파일을 읽지 못했습니다: {msg}", { msg: e instanceof Error ? e.message : String(e) }));
      });
  };

  const submit = async () => {
    if (!chosen) return setError(t("올릴 JSON 파일을 골라 주세요."));
    if (!password) return setError(t("센터 공용 비밀번호를 넣어 주세요."));
    setBusy(true);
    setError("");
    try {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-tt-900/40 p-4" role="dialog" aria-modal="true" aria-label={t("공유에 올리기")}>
      <div className="max-h-[90vh] w-full max-w-lg overflow-y-auto rounded-xl bg-white p-5 shadow-xl">
        <div className="mb-3 flex items-start justify-between gap-3">
          <div>
            <h2 className="text-lg font-bold text-tt-800">{t("시간표 공유에 올리기")}</h2>
            <p className="mt-1 text-sm text-tt-600">
              {t("올리면 이 공유 공간의 모든 사람이 이 시간표를 봅니다.")}
              {shared?.data
                ? t(" 지금 공유 중: {summary} ({ago})", { summary: summary(shared.data, lang), ago: timeAgo(shared.updatedAt, lang) })
                : t(" 아직 공유된 시간표가 없습니다.")}
            </p>
          </div>
          <button type="button" onClick={onClose} className="rounded-full px-2 text-2xl leading-none text-tt-500 hover:bg-tt-50" aria-label={t("닫기")}>
            ×
          </button>
        </div>

        <fieldset className="space-y-2">
          <legend className="mb-1 text-sm font-semibold text-tt-700">{t("무엇을 올릴까요?")}</legend>
          <label className="flex cursor-pointer items-start gap-2 rounded-lg border border-tt-200 p-3 text-sm">
            ...
            <span>
              <b>{t("이 기기에서 구성한 시간표")}</b>
              <br />
              <span className="text-tt-600">{summary(draft, lang)}</span>
            </span>
          </label>
          <label className="flex cursor-pointer items-start gap-2 rounded-lg border border-tt-200 p-3 text-sm">
            <input
            ...
            <span className="min-w-0">
              ...
              <span className="break-all text-tt-600">
                {fileData ? `${fileName} — ${summary(fileData, lang)}` : t("내보내기로 받은 파일을 고릅니다.")}
              </span>
              <br />
