...
export function solve(
  ...
  // "같은 프로그램 구간당 1회"는 (체험반, 프로그램명) 단위로 센다.
  // 같은 프로그램이 여러 강사 배정으로 나뉘어 있어도 한 반의 한 구간에 한 번만 나오게 한다.
  // 기본은 하루 1회. oncePerSegment 인 수업만 구간(dayGroup) 단위로 세어 한 번만 넣는다.
  const comboIndex = new Map<string, number>();
  const lecCombo = new Int32Array(L);
  for (let i = 0; i < L; i++) {
    const key = `${lectures[i].classId}\u0000${lectures[i].subject}`;
    let ci = comboIndex.get(key);
    if (ci === undefined) {
      ci = comboIndex.size;
      comboIndex.set(key, ci);
    }
    lecCombo[i] = ci;
  }
  const COMBO = Math.max(1, comboIndex.size);
  // dayGroup 값을 0..D-1 (같은 묶음의 첫 요일 index)로 정규화해 comboDay 를 D 폭으로 쓴다.
  const dayGroupOf = new Int32Array(D);
  {
    const firstOf = new Map<number, number>();
    for (let d = 0; d < D; d++) {
      const g = req.dayGroup?.[d] ?? d;
      const first = firstOf.get(g);
      if (first === undefined) firstOf.set(g, d);
      dayGroupOf[d] = first ?? d;
    }
  }
  const lecOnce = new Uint8Array(L);
  for (let i = 0; i < L; i++) lecOnce[i] = lectures[i].oncePerSegment ? 1 : 0;
  /** 단위 u 가 슬롯 s 에 놓일 때 "같은 프로그램 1회"를 세는 묶음 index */
  const groupOfSlot = (u: number, s: number) => {
    const day = (s / P) | 0;
    return lecOnce[unitLec[u]] ? dayGroupOf[day] : day;
  };
  ...
  // 요일이 지정된 프로그램(Science 1 은 월요일 등)은 그 요일만 남긴다.
  const lecDayMask: (Uint8Array | null)[] = lectures.map((lec) => {
    const days = lec.days?.filter((d) => d >= 0 && d < D) ?? [];
    if (days.length === 0) return null;
    const mask = new Uint8Array(D);
    for (const d of days) mask[d] = 1;
    return mask;
  });
  ...
  const comboDay = new Int32Array(COMBO * D);
    ...
    for (let c = 0; c < C; c++) {
      ...
      for (const u of list) {
        ...
        for (let i = 0; i < cand.length; i++) {
          ...
          if (comboDay[co * D + groupOfSlot(u, s)] === 0) {
            chosen = s;
            break;
          }
        }
