...
구간 안 반복 & 매일 하는 프로그램이면 켭니다 (아래 설명, 구간을 나눠 두었을 때만 나옵니다) \\
\bottomrule
\end{tabularx}
\end{center}

\begin{warn}
같은 반에서 \textbf{같은 프로그램은 한 구간에 한 번만} 합니다.
월·화에 오는 반이라면 그 프로그램은 월요일이나 화요일 중 \textbf{하루에만} 들어갑니다.\\[2pt]
홈룸 영어나 파닉스처럼 \textbf{매일 하는 프로그램}은 \field{구간 안 반복}을 켜 주세요.
켜면 하루에 한 번씩 여러 날에 나눠 들어갑니다(그래도 같은 날 두 번은 하지 않습니다).
구간을 나누지 않은 반은 예전처럼 하루에 한 번 규칙만 적용됩니다.
\end{warn}
...
\begin{itemize}
  ...
  \item 같은 반의 같은 프로그램은 \textbf{한 구간에 한 번만} 합니다(연속 2교시는 한 번으로 셉니다).
        \field{구간 안 반복}을 켠 프로그램은 하루에 한 번씩 여러 날에 들어갑니다.
...
구간당 1회 규칙 때문에 & 매일 하는 프로그램이면 \field{구간 안 반복}을 켜세요.
그렇지 않다면 연속 2교시 횟수를 늘리거나 시수를 줄입니다. \\
\bottomrule
\end{tabularx}
\end{center}
...
같은 프로그램 중복 & 한 반이 같은 프로그램을 한 구간에 두 번 합니다
(\field{구간 안 반복}을 켠 프로그램이면 같은 날 두 번). \\
