...
\begin{itemize}
  ...
  \item \textbf{모든 체험반과 모든 강사}의 시간표에 노란 칸으로 똑같이 나타납니다(점심시간과 같은 색).
...
\begin{danger}
구간을 나누어도 \textbf{시간표는 하나}입니다. 따로 짜서 나중에 합치는 것이 아니라,
\textbf{한 번에 함께 풀면서} 각 반을 자기 구간의 요일에만 넣습니다.\\
그래서 월·화 팀과 수·목·금 팀이 \textbf{같은 강사·같은 체험존}을 써도 겹치지 않게 잡아 줍니다.
\end{danger}
...
\qa{월·화 시간표와 수·목·금 시간표를 따로 짤 수는 없나요?}{
  따로 짜지 않고 \textbf{한 번에} 짭니다. 대신 각 반은 자기 구간의 요일에만 들어가므로 결과는 같습니다.
  이렇게 하는 편이 더 안전합니다 — 두 구간이 같은 강사·같은 존을 쓸 때 겹치는지까지 함께 봐 주기 때문입니다.
  보거나 인쇄할 때는 구간을 골라 따로 볼 수 있습니다.}
...
\qa{점심시간도 요일마다 다르게 할 수 있나요?}{
  점심시간은 하루 전체에 걸치는 \textbf{시간 띠}라 모든 요일에 똑같이 들어갑니다.
  요일마다 다른 활동이 필요하시면 고정 활동으로 넣으세요.}
